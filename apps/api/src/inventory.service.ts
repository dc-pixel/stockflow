  ...
  async adjust(input: {
    ...
    return this.prisma.$transaction(async (tx) => {
      const product = await tx.product.findFirst({
        where: { id: input.productId, organizationId: input.organizationId, isActive: true },
      });
      ...
      const inventory = await tx.inventory.upsert({
        where: { productId_warehouseId: { productId: input.productId, warehouseId: input.warehouseId } },
        create: { productId: input.productId, warehouseId: input.warehouseId, quantity: newQuantity, reservedQuantity },
      ...
      await tx.auditLog.create({
        data: {
          ...
          before: { quantity: previousQuantity, reservedQuantity },
          after: { quantity: newQuantity, reservedQuantity },
        },
      });
